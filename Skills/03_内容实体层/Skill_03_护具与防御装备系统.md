# 内容实体层：护具与防御装备系统 Skill

## 1. Skill 定位

本 Skill 用于从任意 ACGN 作品名称、Wiki 文档、规则文本、设定集、角色档案、战役记录或自制设定中，抽象生成【护具与防御装备系统】对应的系统 logic。它不直接照搬某一作品的固有术语，而是将不同作品中的近似概念映射到同一套可复用的 TRPG GraphRAG 逻辑结构中。
...
可映射到本系统的常见近似术语包括：护甲、防具、盾、装甲、护盾、防御饰品、结界、屏障、防御模块、装甲部件。

映射原则如下：

1. 若某术语描述的是同一类规则功能，则归入【护具与防御装备系统】。
...
1. 识别作品中的防具、装甲、护盾、屏障、结界或防御饰品。
2. 判断其是否改变防御、抗性、移动、隐匿或施法。
3. 抽取防御值、需求、限制、惩罚和特殊防御效果。
4. 连接角色装备构建与规则执行层防御逻辑。
5. 输出 armor_entity logic，用于命中、防御和减伤判断。
