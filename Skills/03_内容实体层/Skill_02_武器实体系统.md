...
本 Skill 用于从任意 ACGN 作品名称、Wiki 文档、规则文本、设定集、角色档案、战役记录或自制设定中，抽象生成【武器实体系统】对应的系统 logic。它不直接照搬某一作品的固有术语，而是将不同作品中的近似概念映射到同一套可复用的 TRPG GraphRAG 逻辑结构中。
...
可映射到本系统的常见近似术语包括：武器、兵装、武装、装备武器、主武器、副武器、舰装、机体武器、法器、远程武器。

映射原则如下：

1. 若某术语描述的是同一类规则功能，则归入【武器实体系统】。
...
1. 读取作品中的武器、兵装、舰装、法器、机体武装等内容。
2. 判断其是否用于攻击、防御、控制或叙事交互。
3. 抽取武器类别、攻击方式、伤害、射程、限制和附加效果。
4. 建立武器与装备状态、攻击规则、伤害系统的连接。
5. 输出 weapon_entity logic，用于规则执行层调用。
