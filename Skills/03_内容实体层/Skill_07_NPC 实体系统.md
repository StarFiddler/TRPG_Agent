# 内容实体层：NPC 实体系统 Skill

## 1. Skill 定位

本 Skill 用于从任意 ACGN 作品名称、Wiki 文档、规则文本、设定集、角色档案、战役记录或自制设定中，抽象生成【NPC 实体系统】对应的系统 logic。它不直接照搬某一作品的固有术语，而是将不同作品中的近似概念映射到同一套可复用的 TRPG GraphRAG 逻辑结构中。
...
可映射到本系统的常见近似术语包括：NPC、角色、非玩家角色、商人、导师、任务角色、剧情角色、路人、首领、联系人。

映射原则如下：

1. 若某术语描述的是同一类规则功能，则归入【NPC 实体系统】。
...
1. 识别作品中的剧情角色、商人、导师、派系成员或敌对首领。
2. 抽取身份、位置、动机、态度、服务、任务和秘密。
3. 区分 NPC 的叙事属性与可战斗属性。
4. 建立 NPC 与地点、派系、任务、关系网络的连接。
5. 输出 npc_entity logic，用于社交、任务和剧情推进。
